...
export function createStageHandler({ fetchImpl = fetch, dropboxFactory } = {}) {
  const createDropbox = dropboxFactory || ((options) => new Dropbox(options));
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      const base64Parts = image_base64.split(",");
      const base64Data = base64Parts[1] ? base64Parts[1].trim() : "";
      ...
      // === Upload to Dropbox ===
      const dropbox = await createDropbox({ accessToken: process.env.DROPBOX_ACCESS_TOKEN });
